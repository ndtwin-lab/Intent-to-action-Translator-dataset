...
*   **Atomic Intents (Single-step):** For these cases, the `gpt` field represents a direct translation from natural language to a single NDT API call. These are primarily used to evaluate the Edge LLM's mapping accuracy.
*   **Multi-action Intents (Iterative Reasoning):** For complex intents, the `gpt` field represents the Standard (Final Goal). 
